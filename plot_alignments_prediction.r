...
if (length(args)<11) {
    ...
    region end, strand1, strand2, predictions bedfile, gff file, svg/jpeg\n
...
gff <- read.delim(args[10], header=FALSE, comment.char="#")
colnames(gff) <- c("Chrom", "Source", "Type", "Start", "End", "Score",
...
p_svg <- p3 / p1 / p2 + plot_layout(heights = c(5, 1, 1))
